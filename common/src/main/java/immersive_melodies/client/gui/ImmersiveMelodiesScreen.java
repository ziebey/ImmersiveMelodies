...
public class ImmersiveMelodiesScreen extends Screen {
                ...
                if (midiMatcher.matches(path.getFileName())) {
                    ...
                    MinecraftClient.getInstance().execute(() -> {
                        try {
                            ...
                            e.printStackTrace();
                            setError(new TranslatableText("immersive_melodies.error.empty"));
                        }
                    });
                }
            } catch (Exception e) {
    ...
    public void render(MatrixStack matrices, int mouseX, int mouseY, float delta) {
        ...
        if (!Config.getInstance().clickedHelp) {
            renderTooltip(matrices, new TranslatableText("immersive_melodies.read"), width / 2 + 55, height / 2 + 69 + 17);
        }

        // Print error
        if (error != null && System.currentTimeMillis() - lastError < 5000) {
            drawCenteredTextWithShadow(matrices, textRenderer, error.asOrderedText(), width / 2, this.height / 2 - 103, 0xFF0000);
        }
